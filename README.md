...
All four patterns above (plain `irm | iex`, `[scriptblock]::Create(...)` with arguments, plain `curl | bash`, and `bash -s --` with arguments) have been verified end-to-end against the real hosted files, including a live confirmation of the plain `irm | iex` path after fixing a `[ValidateSet]`-vs-`iex` scoping bug it originally hit (see CHANGELOG). Since there's no local file in this mode, `config/mcp-packages.json` isn't reachable by a relative path — both scripts detect that and fall back to their embedded copy of the MCP server list automatically.
